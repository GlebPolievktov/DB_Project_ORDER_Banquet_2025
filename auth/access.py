...
def report_required(func):
    ...
    def wrapper(*args, **kwargs):
        if 'user_group' in session:
            access = current_app.config['db_access']
            user_role = session.get('user_group')
            if 'all' in access[user_role]['check_reports']:
                return func(*args, **kwargs)
            return redirect(url_for('main_error',
                                    message='У вас нет прав на просмотр отчета!',
                                    description='Обратитесь к администратору для получения доступа.'))
        return redirect(url_for('main_error', message='Необходимо авторизоваться!',
